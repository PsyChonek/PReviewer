...
describe('Git Operations Integration', () => {
  ...
  describe('getGitBranches', () => {
    ...
    test('should throw error for non-git directory', async () => {
      ...
      await expect(gitHandlers.getGitBranches(nonGitDir.name))
        .rejects
        .toHaveErrorMessage('not a git repository');
  ...
  describe('Real-world scenarios', () => {
    test('should handle typical feature branch workflow', async () => {
      ...
      await fs.writeFile(path.join(tempDir.name, 'src/logger.js'),
        'export function log(message) { console.log(message); }');
      await fs.writeFile(path.join(tempDir.name, 'src/main.js'),
