		...
		async getGitDiff(repoPath, baseBranch, targetBranch) {
			try {
				...
				if (
					...
					error.message.includes('merge-base') ||
					error.message.includes('no common commits')
				) {
					errorMessage +=
						'\n\nTroubleshooting steps:\n' +
...
describe('Git Operations Integration', () => {
	...
	beforeEach(async () => {
		...
		await fs.writeFile(
			path.join(tempDir.name, 'README.md'),
			'# Test Repository'
		);
	...
	describe('getGitBranches', () => {
		...
		test('should throw error for non-git directory', async () => {
			...
			await expect(
				gitHandlers.getGitBranches(nonGitDir.name)
			).rejects.toHaveErrorMessage('not a git repository');
		...
		test('should throw error for non-existent directory', async () => {
			...
			await expect(
				gitHandlers.getGitBranches(nonExistentPath)
			).rejects.toHaveErrorMessage(
		...
		test('should include troubleshooting for permission errors', async () => {
			...
			const mockGit = {
				branchLocal: jest
					.fn()
					.mockRejectedValue(
						new Error('permission denied accessing repository')
					),
			};
			...
			try {
				...
				await expect(
					testHandlers.getGitBranches(restrictedDir.name)
				).rejects.toHaveErrorMessage('Check folder permissions');
			} finally {
				// Restore original
	...
	describe('getGitDiff', () => {
		test('should generate diff between branches', async () => {
			// Create file on main
			await fs.writeFile(
				path.join(tempDir.name, 'test.js'),
				'console.log("hello");'
			);
			...
			const diff = await gitHandlers.getGitDiff(
				tempDir.name,
				'main',
				'feature'
			);
		...
		test('should throw error for non-existent branches', async () => {
			...
			await expect(
				gitHandlers.getGitDiff(tempDir.name, 'main', 'non-existent')
			).rejects.toHaveErrorMessage('Not a valid object name');
		});
		...
		test('should provide troubleshooting for bad revision errors', async () => {
			...
			await expect(
				gitHandlers.getGitDiff(tempDir.name, 'invalid-branch', 'main')
			).rejects.toHaveErrorMessage('Not a valid object name');
		});
	...
	describe('Error handling edge cases', () => {
		test('should handle corrupted git repository', async () => {
			...
			await fs.writeFile(
				path.join(tempDir.name, '.git', 'HEAD'),
				'invalid content'
			);
		...
		test('should handle very large repository operations', async () => {
			...
			await git.checkoutLocalBranch('feature');
			await fs.writeFile(
				path.join(tempDir.name, 'large.txt'),
				largeContent + '\nadded line'
			);
			await git.add('large.txt');
	...
	describe('Real-world scenarios', () => {
		test('should handle typical feature branch workflow', async () => {
			...
			await fs.writeFile(
				path.join(tempDir.name, 'src/main.js'),
			...
			await fs.writeFile(
				path.join(tempDir.name, 'src/logger.js'),
