...
contextBridge.exposeInMainWorld('electronAPI', {
	...
	getGitBranches: (repoPath: string): Promise<string[]> =>
		ipcRenderer.invoke('get-git-branches', repoPath),
	...
	callOllamaAPI: (config: {
		...
		model: string;
		prompt: string;
	}): Promise<string> => ipcRenderer.invoke('call-ollama-api', config),
	...
	callAzureAI: (config: {
		...
		prompt: string;
	}): Promise<string> => ipcRenderer.invoke('call-azure-ai-api', config),
	...
	// Progress listeners
	onOllamaProgress: (
		callback: (event: IpcRendererEvent, data: ProgressData) => void
	): (() => void) => {
