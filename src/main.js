...
ipcMain.handle(
	'get-git-diff',
	async (event, repoPath, baseBranch, targetBranch) => {
		try {
			const git = simpleGit(repoPath);
...
ipcMain.handle(
	'call-azure-ai-api',
	async (event, { endpoint, apiKey, deploymentName, prompt }) => {
		try {
			// Send initial progress
...
ipcMain.handle(
	...
	async (event, { endpoint, apiKey, deploymentName }) => {
		try {
			// Create Azure OpenAI client using the stable OpenAI SDK
			// Extract base URL from the full endpoint if it contains the full path
			let baseURL = endpoint;
